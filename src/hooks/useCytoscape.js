...
export const useCytoscape = ({
  ...
  }, [pathData, wrappedTokens, containerRef, onTooltip, onTransactionSelect]);
...
// Helper function to create a Cytoscape instance
function createCytoscapeInstance({ containerRef, pathData, wrappedTokens, onTooltip, onTransactionSelect }) {
  ...
  pathData.transfers.forEach(transfer => {
    ...
    const ownerLabel = `${transfer.tokenOwner.slice(0, 6)}...${transfer.tokenOwner.slice(-4)}`;
